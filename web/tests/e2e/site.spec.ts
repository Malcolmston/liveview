  ...
  for (const id of TAB_IDS) {
    ...
    for (let i = 0; i < count; i++) {
      ...
      if (/^https?:\/\//.test(href)) {
        ...
        // /liveview/api/. It is produced by the Go doc generator at deploy time,
        ...
        expect(resolved.pathname, `docs link should resolve under /liveview/`).toBe('/liveview/api/');
      } else {
...
test('docs tab renders the React API reference with a package listed', async ({ page }) => {
  ...
  // liveview is a single-package module (package liveview), so at least one package link is expected.
